...
function WorkflowBuilder() {
  ...
  // Always-current ref to nodes so callbacks can read latest nodes without
  // depending on `nodes` (which would otherwise cause setNodes->effect loops).
  ...
  const handleExecutionClick = useCallback(
    async (nodeId) => {
      ...
      // For chat trigger nodes, open the chat UI instead of executing.
      ...
      setNodeExecState({ status: "running", output: "Executing...", startTime, endTime: null });

      let tempWorkflowId = null;
      try {
        // Build a temporary workflow: a manual trigger feeding this node.
        ...
        const testWorkflow = {
          name: "__node_test__",
          description: "Temporary workflow for single-node testing",
          nodes: [
            {
              id: "test-trigger",
              type: "manual-trigger",
        ...
        const created = await apiService.createWorkflow(testWorkflow);
        tempWorkflowId = created.id;

        const result = await apiService.executeWorkflow(created.id, {
        ...
        if (failed) {
          const errorMessage =
            result.error || nodeState?.error || "Node execution failed";
          setNodeExecState({ status: "error", output: `❌ ${errorMessage}`, startTime, endTime });
          ...
          const output = nodeState?.output ?? result.execution?.chat_response ?? "Success";
          const outputText =
            typeof output === "string" ? output : JSON.stringify(output, null, 2);
          setNodeExecState({ status: "success", output: outputText, startTime, endTime });
          setExecutionHistory((prev) => [
            {
              id: Date.now() + Math.random(),
              nodeType: node.data.type,
          ...
          showToast(`✅ ${node.data.label} test passed`, "success", 3000);
        }
      } catch (error) {
        const endTime = new Date();
        ...
        if (tempWorkflowId) {
          try {
            await apiService.deleteWorkflow(tempWorkflowId);
          } catch {
    ...
    [loadNodesWithProperties, setExecutingNodes, showToast]
  );
