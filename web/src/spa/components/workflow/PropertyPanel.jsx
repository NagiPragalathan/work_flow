...
const PropertyPanel = ({ node, onUpdate, onClose }) => {
  ...
  const handlePropertyChange = (propKey, value) => {
    ...
    try {
      ...
      console.log(`💾 Saved ${propKey} to localStorage for node ${nodeId}`);
    } catch (error) {
  ...
  // Function to sync all properties to localStorage before closing
  const handleClose = () => {
    if (node?.id && Object.keys(inputValues).length > 0) {
      try {
        localStorage.setItem(`inputValues_${node.id}`, JSON.stringify(inputValues));
        console.log(`💾 Final sync to localStorage for node ${node.id}:`, Object.keys(inputValues));
      } catch (error) {
